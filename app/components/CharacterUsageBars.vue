...
import type { UsageRow } from '../composables/useStatsRows';
  ...
  <div v-if="compact" ref="root" data-testid="usage-bars" class="flex flex-col gap-3">
    ...
    >
      <div class="mb-[5px] flex items-center gap-2">
        <CharacterBadge :character-id="row.id" :size="26" :notch="6" :font-size="10" />
        ...
        <span class="ml-auto font-mono text-[11px] text-text-secondary">
          <span class="count-up" :data-value="row.value">{{
  ...
  <div v-else ref="root" data-testid="usage-bars" class="flex flex-col gap-2 md:gap-[9px]">
    <div v-for="row in shown" :key="row.id" class="flex items-center gap-2.5 md:gap-3">
      <span
        ...
        >{{ row.rank }}</span
      >
      <CharacterBadge :character-id="row.id" :size="28" :notch="7" :font-size="11" />
      <span
